...
export default function ChatFull() {
  ...
  const handleShareClick = async (e, chatId) => {
    ...
    if (link) {
      ...
      setToastMessage('¡Enlace de compartición copiado al portapapeles! 🔗')
          ...
          <div className="projects-dashboard__main-col">
            ...
            <div className="projects-section" style={{ marginTop: '24px' }}>
              <h2 className="projects-section__title">
                {activeTab === 'compartidos' ? 'Proyectos compartidos contigo' : 'Todos los proyectos'}
              </h2>
              {proyectosAMostrar.length === 0 ? (
                <div className="projects-dashboard__empty" style={{ padding: '60px 20px', border: '1.5px dashed #cbd5e1', borderRadius: '16px', background: '#f8fafc', textAlign: 'center' }}>
                  <p style={{ fontSize: '15px', color: '#475569', fontWeight: '600', marginBottom: '8px' }}>
                    {activeTab === 'compartidos' ? 'No tienes proyectos compartidos.' : 'No tienes chats.'}
                  </p>
                  <p style={{ fontSize: '13px', color: '#64748b' }}>
                    {activeTab === 'compartidos' ? 'Pídele a tus compañeros que te compartan sus enlaces.' : 'Empieza una nueva conversación para visualizarla acá.'}
                  </p>
                </div>
              ) : (
                <div className="projects-grid">
                  ...
                  sharedSessions.slice(0, 3).map((sess) => (
                    ...
                    >
                      ...
                      <div className="shared-project-info">
                        ...
                        <span>Ver circuito compartido</span>
                      </div>
                      <span className="shared-project-tag">LECTURA</span>
                    </div>
                  ))
                ) : (
                  <div className="widget-empty-placeholder" style={{ padding: '12px 4px', fontSize: '12px', color: '#94a3b8' }}>
                    No hay proyectos compartidos contigo recientemente.
                  </div>
                )}
              </div>
            </div>
